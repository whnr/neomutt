...
static int help_doclist_parse(struct Context *ctx)
{
  ...
  ctx->mailbox->hdrs = (struct Email **) (help_list_clone(DocList, true, help_doc_clone))->data;
  ctx->mailbox->msg_count = ctx->mailbox->hdrmax = DocList->size;
  ...
  if (request)
  {
    ...
    { /* TODO: prioritise folder (chapter/section) over root file names */
      if (mutt_str_strncmp(ctx->mailbox->hdrs[i]->path, request, mutt_str_strlen(request)) == 0)
      {
        ctx->mailbox->hdrs[0]->read = true;
...
static int help_mbox_open(struct Context *ctx)
{
...
static int help_mbox_close(struct Context *ctx)
{
...
static int help_msg_open(struct Context *ctx, struct Message *msg, int msgno)
{
  mutt_debug(1, "entering help_msg_open: %d, %s\n", msgno, ctx->mailbox->hdrs[msgno]->env->subject);

  char path[PATH_MAX];
  snprintf(path, sizeof(path), "%s/%s", ctx->mailbox->realpath, ctx->mailbox->hdrs[msgno]->path);

  ctx->mailbox->hdrs[msgno]->read = true; /* reset a probably previously set unread status */
...
static int help_msg_open_new(struct Context *ctx, struct Message *msg, struct Email *e)
{
...
static int help_tags_commit(struct Context *ctx, struct Email *e, char *buf)
{
